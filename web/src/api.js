...
export const api = {
  ...
  growth: (params) => request(`/api/growth?${new URLSearchParams(params)}`),
  goals: () => request('/api/goals'),
  week: (key) => request(`/api/weeks/${key}`),
  tasks: () => request('/api/coaching-tasks'),
  addTask: (data) => request('/api/coaching-tasks', { method: 'POST', body: JSON.stringify(data) }),
  patchTask: (id, status) => request(`/api/coaching-tasks/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  addCheckpoint: (data) => request('/api/checkpoints', { method: 'POST', body: JSON.stringify(data) }),
  decideCheckpoint: (id, decision) => request(`/api/checkpoints/${id}`, { method: 'PATCH', body: JSON.stringify({ decision }) }),
  ...
  sync: (provider) => request(`/api/integrations/${provider}/sync`, { method: 'POST' }),
